...
class MainActivity : FlutterFragmentActivity() {
  ...
  val deviceName = BluetoothAdapter.getDefaultAdapter()?.name ?: "Unknown Device"
  val deviceNameBytes = deviceName.toByteArray(Charsets.UTF_8)
  ...
  lastDevice?.let { device ->
    pkcsChar?.let { charac ->
      ...
      // Then send device name separately
      gattServer?.notifyCharacteristicChanged(device, charac, false, deviceNameBytes)
      Log.i("BLE", "Device name notified (${deviceNameBytes.size} bytes): $deviceName")
    }
  }
  result.success(null)
}
